...
function get_team_members($conn, $team_id) {
  $stmt = $conn->prepare("SELECT members.firstname, members.lastname, members.study, members.email, members.image, members.linkedin, member_in_team.position_description FROM members, member_in_team WHERE member_in_team.team_id = ? AND member_in_team.member_id = members.member_id ORDER BY member_in_team.order ASC");
  $stmt->bind_param('i', $team_id);
  $stmt->execute();
  $result = $stmt->get_result();
  if ($result->num_rows === 0) {
